...
static final float MARCH_BPM = 35.793;
static final int MARCH_FRAMES = (int) (PHYS_FPS * 60.0 / MARCH_BPM);
...
static final float STEER_FACTOR = 0.02;
...
boolean inRadius(float ax, float ay, float bx, float by, float rad)
{
...
{
  float y = height;
  float x = width/2 - ((NUM_CITIES-1)/2.0)*CITY_SPACING + c*CITY_SPACING;
...
rectMode(CORNER);
  ...
  {
    if (strength <= 0)
      missiles.add(new Missile(new PVector(pos.x, pos.y+INVADER_RADIUS), new PVector(0, 0.6), false)); //straight down
    ...
    if (ml == 3 ) {
     ...
     // laser3.setLooping(true);
      ...
      missiles.add(new Missile(new PVector(pos.x, pos.y+INVADER_RADIUS), new PVector(random(0.6)-0.3, 0.6), false)); //scatter
    ...
    { //aim fast bank shot at a living base
      ...
      vel.mult(1.5);
      missiles.add(new Missile(origin, vel, false));
    } else missiles.add(new Missile(new PVector(pos.x, pos.y+INVADER_RADIUS), new PVector(random(4)-2, 1.7), false)); //fast random shot
  }
  void getHit()
  {
...
void drawHud()
{
 ...
 if (waveTimer < ANNOUNCE_TIME || gameOver  )
  ...
  if (paused)
  {
    ...
    if (flawlessbadge == true && score > 500000) {
      image(FlawlessBadge, 0, 0);
      newcity.start();
    }
    if ( doomsdaybadge == true && waveTimer > 1) {
...
void mousePressed()
{
  ...
  if (mousePressed) {
     ...
     screencheck = screencheck+1 ;
     
    if (mousePressed && screencheck == 2) {
      
    if (mouseX>chx-150 && mouseX <chx+260 && mouseY>chy-25 && mouseY <chy+70 && atTitle ) {
      {
        //screencheck = 0;
        doomsday = false;
        nightmare = true; 
    ...
    if (mouseX>fbx && mouseX <fbx+80 && mouseY>fby && mouseY <fby+120 && atTitle ) {
      {
        
        link("https://www.facebook.com/MADXMISSILE?ref=bookmarks");
        //screencheck = 2; 
        ///facebook share
      }
    }
      ...
      if (mouseX>ttx-190 && mouseX <ttx+340 && mouseY>tty-10 && mouseY <tty+70 && atTitle  ) {
        {
          //screencheck = 0;
          doomsday = true;
          nightmare = false; 
